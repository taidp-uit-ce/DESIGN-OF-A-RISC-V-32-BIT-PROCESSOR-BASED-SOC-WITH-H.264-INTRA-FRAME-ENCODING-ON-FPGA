...
%           disp('Prediciton block size 4x4')
           ...
           for m = i:4:i+15
               for n = j:4:j+15
                   idx1_temp = idx;%taidao
                   [mode_diff,idx]= dec_golomb(idx,bitstream,1);
                   %fprintf('dec_golomb=%d - mode=%d\n', idx -idx1_temp, mode_diff);
                   mode = mode_prev + mode_diff;
                   [S(m:m+3,n:n+3),idx] = dec_mb_4(idx,bitstream,mode,S,m,n);
                   idx2_temp = idx;%taidao
                   mode_prev = mode;
                   idx3_temp = idx2_temp - idx1_temp;%taidao
                   %if(i==1 && j==1)
                   %     fprintf('decode_block4x4[%d:%d]: mode=%d - length(bits)=%d\n', m, n, mode, idx3_temp); %taidao
                   %end
                   %if(m==1 && n==9)
                   %     idx = idx -4;
                   %end
