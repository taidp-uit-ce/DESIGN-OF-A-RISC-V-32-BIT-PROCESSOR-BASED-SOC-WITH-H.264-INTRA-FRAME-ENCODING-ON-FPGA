function [S,idx] = i_frame_dec_v2(idx,bitstream)
% global variables
global QP h w
...
for i = 1:bs:h
    for j = 1:bs:w
        if bitstream(idx)=='0'
           mb_type = 0;
           % disp('Prediction block size 16x16')
           idx = idx + 1;
        ...
        elseif  bitstream(idx)=='1'
           mb_type = 1;
           % disp('Prediction block size 4x4')
           idx = idx + 1;
           for m = i:4:i+15
               for n = j:4:j+15
                   [mode_diff,idx]= dec_golomb(idx,bitstream,1);
...
pred = (sum(Seq_r(i-1,j:j+15))+ sum(Seq_r(i:i+15,j-1))+16)/32; %taidao
...
% b = bitshift(uint8(round(5*H + 32)),-6);%%taidao add uint8(round())
b = (5*H + 32)/64;
% b = bitshift(5*H + 32,-6);
% c = bitshift(uint8(round(5*V + 32)),-6); %%taidao add uint8(round())
c = (5*V + 32)/64;
% c = bitshift(5*V + 32,-6);
