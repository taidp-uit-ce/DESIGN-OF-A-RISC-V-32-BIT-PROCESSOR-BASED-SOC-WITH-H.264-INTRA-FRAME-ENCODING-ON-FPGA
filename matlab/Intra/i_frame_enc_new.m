function [cur_frame_ref, bits_frame] = i_frame_enc_new(cur_frame, Quant)
    global QP h w
    ...
    %% U
    [bits_u, cur_frame_u_ref, n_a] = chroma_prediction(cur_frame(:,:,2));
    bits_frame = [bits_frame bits_u];
    %% V
    [bits_v, cur_frame_v_ref, n_a] = chroma_prediction(cur_frame(:,:,3));
...
function [bits_frame, cur_frame_ref] = luma_prediction (cur_frame)
    ...
    for i = 1:16:h
        for j = 1:16:w
            [bits_4x4, cur_frame_ref4x4, sae1, mode4] = intra_4(cur_frame, mode_prev, i, j);
            [bits_16x16, cur_frame_ref16x16, sae2, mode16] = intra_16(cur_frame, mode_prev, i, j);
            if (sae1 < sae2)
                fprintf("ENCODER(%d, %d): CHOSE INTRA 4", i, j);
                bits_frame = [bits_frame bits_4x4];
                cur_frame_ref(i:i+15, j:j+15)= cur_frame_ref4x4;
                mode_prev = mode4;
            else
                fprintf("ENCODER(%d, %d): CHOSE INTRA 16", i, j);
                bits_frame = [bits_frame bits_16x16];
                cur_frame_ref(i:i+15, j:j+15) = cur_frame_ref16x16;  
                mode_prev = mode16;
            end
        end
    end
...
function [bits_frame, cur_frame_ref, total_sae, mode_out] = intra_4(cur_frame, mode_prev_in, m, n)
    global h w
    bits_frame = '';
    total_sae = 0;
    mode_prev = 0;
    ...
    for i = m:4:m+15
        for j = n:4:n+15
            if (i==1)&(j==1)    % No prediciton
                ...
                bits = enc_golomb(mode - mode_prev_in, 1); %taidao
                mode_prev = mode;
...
function [icp,pred,sae] = pred_ddl_4(cur_frame,cur_frame_ref,i,j)
    ...
    for x = 0:3
        for y = 0:3
            if (x==3)&(y==3)
                pred(x+1,y+1) = bitshift(cur_frame_ref(i+6,j-1) + 3*cur_frame_ref(i+7,j-1) + 2,-2);
            else
...
function [icp,pred,sae] = pred_hd_4(cur_frame,cur_frame_ref,i,j)
    for x = 0:3
        for y = 0:3
            ...
            w = bitshift(x,-1);
            if rem(z,2)==0
                pred(x+1,y+1)= bitshift(cur_frame_ref(i-1,j+y-w-1) + cur_frame_ref(i-1,j+y-w) + 1,-1);
            elseif rem(z,2)==1
                pred(x+1,y+1)= bitshift(cur_frame_ref(i-1,j+y-w-2) + 2*cur_frame_ref(i-1,j+y-w-1) + cur_frame_ref(i-1,j+y-w) + 2,-2);
            elseif z==-1
                pred(x+1,y+1)= bitshift(cur_frame_ref(i-1,j)+ 2*cur_frame_ref(i-1,j-1) + cur_frame_ref(i,j-1) + 2,-2);
            else
                pred(x+1,y+1) = bitshift(cur_frame_ref(i+x-1,j-1)+ 2*cur_frame_ref(i+x-2,j-1) + cur_frame_ref(i+x-3,j-1) + 2,-2);
            end
        end
    end
...
function [icp,pred,sae] = pred_vl_4(cur_frame,cur_frame_ref,i,j)
    ...
    for x = 0:3
        for y = 0:3
            w = bitshift(y,-1);
            if rem(y,2)==0
                pred(x+1,y+1) = bitshift(cur_frame_ref(i+x+w,j-1) + cur_frame_ref(i+x+w+1,j-1) + 1,-1);
            else
                pred(x+1,y+1) = bitshift(cur_frame_ref(i+x+w,j-1) + 2*cur_frame_ref(i+x+w+1,j-1) + cur_frame_ref(i+x+w+2,j-1) + 2,-2);
            end
        end
    end
...
function [icp,pred,sae] = pred_hu_4(cur_frame,cur_frame_ref,i,j)
    for x = 0:3
        for y = 0:3
            ...
            w = bitshift(x,-1);
            if (z==0)|(z==2)|(z==4)
                pred(x+1,y+1)= bitshift(cur_frame_ref(i-1,j+y+w) + cur_frame_ref(i-1,j+y+w+1) + 1,-1);
            elseif (z==1)|(z==3)
                pred(x+1,y+1)= bitshift(cur_frame_ref(i-1,j+y+w) + 2*cur_frame_ref(i-1,j+y+w+1) + cur_frame_ref(i-1,j+y+w+2) + 2,-2);
            elseif z==5
                pred(x+1,y+1)= bitshift(cur_frame_ref(i-1,j+2)+ 3*cur_frame_ref(i-1,j+3) + 2,-2);
            else
                pred(x+1,y+1) = cur_frame_ref(i-1,j+3);
            end
        end
    end
...
%--------------------------------------------------------------
function [bits_frame, cur_frame_ref, total_sae, mode_out] = intra_16(cur_frame, mode_prev_in, i, j)
    global h w
    bits_frame = '';
    total_sae = 0;
    mode_prev = mode_prev_in;
    mode = 0;
...
function [bits_frame, cur_frame_ref, total_sae] = chroma_prediction(cur_frame)
    global h w
    bits_frame = '';
    total_sae = 0;
    mode_prev = 0;
